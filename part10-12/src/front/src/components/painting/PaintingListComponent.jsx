...
const PaintingListComponent = (props) => {
  ...
  const [selectedPaintings, setSelectedPaingtings] = useState([]);
  ...
  const deletePaintingsClicked = () => {
    ...
    if (x.length > 0) {
      ...
      setShowAlert(true);
      setSelectedPaingtings(x);
      setMessage(msg);
    }
  };

  const refreshPaintings = (currentPage) => {
    BackendService.retrieveAllPaintings(currentPage, limit)
      .then((resp) => {
  ...
  const updatePaingtingClicked = (id) => {
  ...
  const onPageChanged = (currentPage) => {
    refreshPaintings(currentPage - 1);
  };
                    ...
                    <div className="btn-toolbar">
                      <div className="btn-group  ms-auto">
                        <button
                          ...
                          onClick={() => updatePaingtingClicked(painting.id)}
                        >
